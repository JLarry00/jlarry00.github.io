...
La colección tiene una sola entrada pública en `/for-agents/`. Su índice y los 22 documentos ES/EN se escriben en Markdown en `src/content/agent-docs/` y se publican como HTML estático, sencillo y enlazado. Personas y agentes recorren los mismos enlaces; un agente no necesita ejecutar el workbench visual. Los `.md` fuente no se publican como una segunda vista. Welcome y cada ficha enlazan a la colección.
